...
bool vision_msgs__msg__armors__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("vision_msgs.msg._armors.Armors", full_classname_dest, 30) == 0);
  }
  vision_msgs__msg__Armors * ros_message = _ros_message;
  {  // header
  ...
  {  // armors
    ...
    if (!vision_msgs__msg__Armor__Sequence__init(&(ros_message->armors), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create vision_msgs__msg__Armor__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    vision_msgs__msg__Armor * dest = ros_message->armors.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!vision_msgs__msg__armor__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
PyObject * vision_msgs__msg__armors__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("vision_msgs.msg._armors");
  ...
  {  // armors
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      PyObject * pyitem = vision_msgs__msg__armor__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
